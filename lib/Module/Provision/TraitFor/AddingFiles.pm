...
around 'generate_metadata' => sub {
   ...
   my $mdf = $orig->($self, @args);

   $self->add_to_vcs($mdf) if $mdf and $self->appldir->catfile($mdf)->exists;
...
sub module : method {
   my $self   = shift;
   my $target = $self->_get_target('libdir', \&classfile);

   $self->output('Adding new module') unless $self->quiet;
...
sub program : method {
   my $self   = shift;
   my $target = $self->_get_target('binsdir');

   $self->output('Adding new program') unless $self->quiet;
...
sub update_file : method {
   ...
   throw 'File [_1] not in template map', [$target]
      unless exists $index->{$target};

   my $source = $self->template_dir->catfile($index->{$target});

   throw 'File [_1] not found', [$source] unless $source->exists;
...
sub _get_target {
   ...
   $self->stash->{abstract} = $abstract if $self->method ne 'test';

   return $target;
}

sub _add_test_script {
   my $self   = shift;
   my $target = $self->_get_target('testdir');

   $self->output( 'Adding new test' ) unless $self->quiet;
